...
export async function POST(request: Request) {
  try {
    ...
    const { building, phoneSearch, template } = body;
    ...
    const leads = data.map((row: any) => {
      ...
      message = message
        ...
        .replace(/\s+/g, ' ').trim();
      return { phone: row.phone, owner_name: row.owner_name, building_name: row.building_name, unit_number: row.unit_number, message };
    });

    const n8nResponse = await fetch(webhookUrl, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ campaign_name: building || 'General Campaign', total_leads: leads.length, leads })
    });

    return NextResponse.json({ success: true, total_sent: leads.length, n8n_status: n8nResponse.status, sample_message: leads[0]?.message });
  } catch (err: any) {
